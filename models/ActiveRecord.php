...
class ActiveRecord
{
    ...
    public static function paginate($porPagina, $order = 'ASC')
    {

        $urlActual = $_SERVER['REQUEST_URI'] ?? '/';

        if (!isset($_GET['pagina'])) {
            // Redirigir a la misma página con el parámetro ?pagina=1
            header("Location: $urlActual?pagina=1");
            exit();
        }

        $pagina = validarORedireccionar('/admin', 'pagina');
        $desde = ($pagina - 1) * $porPagina;
        $totalRegistros = self::count();

        $query = "SELECT * FROM " . static::$tabla . " ORDER BY id " . $order . " LIMIT " . $desde . "," . $porPagina;
        $resultados = self::consultarSQL($query);
        $paginador = new Paginador($resultados, $totalRegistros, $porPagina, $pagina);
        return $paginador;
    }
